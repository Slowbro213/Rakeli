...
**Listeners**Listeners are basically applications running on the C2 server that wait for a callback over a specific port or protocol. Some examples of this are **DNS, HTTP or HTTPS.**
...
5. C2 Beaconing initializes, and the threat actor can then engage with the victim on the C2 server.
...
1. The C2 server then sends commands back to the Victim in the non-restricted network segment.

1. The Victim in the non-restricted network segment then forwards the C2 instructions to the hosts in the restricted segment
...
1. Clone the repository from Gitlab at https://gitlab.com/kalilinux/packages/armitage.git

1. “cd” into Armitage and then build the release using the command “bash package.sh”

1. Check that Armitage is successfully built with “cd ./release/unix/ && ls - la”

1. **Note: It is important that Java SDK 11 is installed and selected, newer versions dont work**
...
Start with this command:**teamserver <ip> <password>**
...
### root@kali$ ssh -L 55553:127.0.0.1:55553 root@<local ip>
